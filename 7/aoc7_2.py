...
class HandType(Enum):
    ...
    def findType(cls,cards:str)->'HandType':
        ...
        for card in sortedCards:
            ...
            lastCard = card
        matchesInHand.append(ofSame)
        matchesInHand.pop(0) #first element compares to "#"
            
        if(5-numberOfJokers in matchesInHand):
            return HandType.FIVEOFAKIND
        if(4-numberOfJokers in matchesInHand):
            return HandType.FOUROFAKIND
        if(3 in matchesInHand and 2 in matchesInHand):
            return HandType.FULLHOUSE
        if(3-numberOfJokers in matchesInHand):
            return HandType.THREEOFAKIND
        if(matchesInHand.count(2) == 2):
            return HandType.TWOPAIR
        if(matchesInHand.count(2)-numberOfJokers == 1):
...
class Hand:
    ...
    def __repr__(self):
        return "Cards: '{cards}' {bet} type:{t}\n".format(cards=self.cards, bet=self.bet, t=self.handType)
